            ...
            <article v-for="child in children" :key="child.id" class="teachify-card rounded-[1.6rem] p-5">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm font-bold text-teachify-muted">
                            {{ child.groups.length }} {{ child.groups.length === 1 ? 'group' : 'groups' }}
                        </p>
                    </div>
                    <span
                        v-if="child.code"
                ...
                <div v-if="child.groups.length" class="mt-5 space-y-4">
                    ...
                    >
                        ...
                        <p v-else class="mt-3 text-sm font-medium text-teachify-muted">No sessions recorded yet.</p>
                    </div>
                </div>
                <p v-else class="mt-4 text-sm font-medium text-teachify-muted">No groups assigned yet.</p>
            </article>
