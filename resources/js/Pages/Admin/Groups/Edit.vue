...
<template>
    <Head :title="$t('admin.groups.edit')" />
    <AppShell :title="$t('admin.groups.edit')">
        <form class="teachify-card max-w-3xl space-y-5 rounded-[1.6rem] p-5" @submit.prevent="submit">
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <TextInput v-model="form.subject" :label="$t('fields.subject')" :error="form.errors.subject" />
                <TextInput v-model="form.level" :label="$t('fields.level')" :error="form.errors.level" />
            </div>
            ...
            <TextareaInput v-model="form.description" :label="$t('fields.description')" :error="form.errors.description" />
            <section>
                <h2 class="text-sm font-black">{{ $t('fields.students') }}</h2>
            ...
            <div class="flex gap-3">
                <Button type="submit" :disabled="form.processing">{{ $t('admin.groups.save') }}</Button>
