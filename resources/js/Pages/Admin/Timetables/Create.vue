...
<template>
    <Head :title="$t('admin.timetables.add')" />
    <AppShell :title="$t('admin.timetables.add')">
        <form v-if="groups.length" class="teachify-card max-w-4xl space-y-5 rounded-[1.6rem] p-5" @submit.prevent="submit">
            <div>
                ...
                <p class="mt-1 text-sm font-medium text-teachify-muted">{{ $t('admin.timetables.createHelp') }}</p>
            </div>
            <SearchableSelect
                v-model="form.teaching_group_id"
                :label="$t('fields.group')"
                :options="groups"
                :placeholder="$t('forms.searchAndSelect')"
            ...
            <section>
                <div class="flex items-center justify-between gap-3">
                    ...
                    <p class="text-sm font-medium text-teachify-muted">{{ $t('admin.timetables.scheduleHelp') }}</p>
                </div>
                    ...
                    <article v-for="(entry, index) in form.entries" :key="entry.day" class="rounded-[1.4rem] border border-teachify-line bg-white p-4">
                        <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto_minmax(0,0.9fr)_minmax(0,0.9fr)] md:items-center">
                            <div>
                                ...
                                <div class="mt-1 text-xs font-medium text-teachify-muted">{{ $t('admin.timetables.activeDayHelp') }}</div>
                            </div>
            ...
            <Button type="submit" :disabled="form.processing">{{ $t('admin.timetables.create') }}</Button>
        </form>

        <EmptyState v-else :title="$t('admin.timetables.allGroupsHaveTimetables')" :message="$t('admin.timetables.allGroupsHaveTimetablesMessage')" />
    </AppShell>
</template>
