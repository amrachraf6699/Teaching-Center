    ...
    <AppShell title="Student Home">
        <section class="overflow-hidden rounded-[1.8rem] border border-teachify-line bg-[linear-gradient(135deg,rgba(37,99,235,0.08),rgba(255,255,255,0.95)_42%,rgba(251,191,36,0.08))] shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
            <div class="flex flex-col gap-4 p-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="max-w-2xl">
                    ...
                    <p class="mt-2 text-sm font-medium text-teachify-muted">
                        Student code <span class="font-black text-teachify-ink">{{ student.code }}</span>
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <Link :href="page.props.routes.studentSessions" class="rounded-2xl border border-white/70 bg-white/85 px-4 py-4 text-sm font-bold text-teachify-ink transition hover:border-teachify-blue hover:text-teachify-blue">
                        <div class="flex items-center gap-2"><CalendarDays class="h-4 w-4" /> This week</div>
                    ...
                    <Link :href="page.props.routes.studentExams" class="rounded-2xl border border-white/70 bg-white/85 px-4 py-4 text-sm font-bold text-teachify-ink transition hover:border-teachify-blue hover:text-teachify-blue">
                        <div class="flex items-center gap-2"><FileText class="h-4 w-4" /> Exams</div>
                        <div class="mt-2 text-xs font-semibold text-teachify-muted">{{ student.upcoming_exams.length }} upcoming · {{ student.conducted_exams.length }} conducted</div>
                    </Link>
                </div>
            </div>
            ...
            <div class="border-t border-white/70 bg-white/75 px-5 py-4 sm:px-6">
                <div class="grid gap-3 sm:grid-cols-4">
                    ...
                    <div class="rounded-2xl border border-teachify-line bg-white px-4 py-3">
                        <div class="text-xs font-black uppercase tracking-[0.16em] text-teachify-muted">Upcoming sessions</div>
                        <div class="mt-1 text-sm font-bold text-teachify-ink">{{ student.upcoming_sessions.length }}</div>
                    </div>
                    <div class="rounded-2xl border border-teachify-line bg-white px-4 py-3">
                        <div class="text-xs font-black uppercase tracking-[0.16em] text-teachify-muted">Recent attendance</div>
                        <div class="mt-1 text-sm font-bold text-teachify-ink">{{ student.attendance.length }}</div>
                    </div>
                    <div class="rounded-2xl border border-teachify-line bg-white px-4 py-3">
                        <div class="text-xs font-black uppercase tracking-[0.16em] text-teachify-muted">Notifications</div>
        ...
        <section class="mt-6 grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
            <div class="teachify-card rounded-[1.6rem] p-5">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        <h2 class="text-lg font-black text-teachify-ink">Upcoming sessions</h2>
                        <p class="text-sm font-medium text-teachify-muted">Your next scheduled lessons.</p>
                    </div>
                    <Link :href="page.props.routes.studentSessions" class="text-sm font-bold text-teachify-blue">View all</Link>
                </div>
                ...
                <EmptyState v-else title="No upcoming sessions" message="Your next lessons will appear here." />
            </div>
            ...
            <div class="teachify-card rounded-[1.6rem] p-5">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-sm font-medium text-teachify-muted">Available and scheduled exams.</p>
                    </div>
                    <Link :href="page.props.routes.studentExams" class="text-sm font-bold text-teachify-blue">Open exams</Link>
                </div>
        ...
        <section class="mt-6 grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
            <div class="teachify-card rounded-[1.6rem] p-5">
                ...
                <EmptyState v-else title="No attendance yet" message="Attendance records will appear here." />
            </div>
